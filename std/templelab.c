/*Adapted from templemisc to support temple 
warded mage labs - Octothorpe 7/7/11*/
...
void trigger_ward() {
   ...
   for(i = 0;i< sizeof(combatants); i++) {
     if(sizeof(combatants[i]->query_attackers()) ||
present("templewardxxx",combatants[i])->is_combatant()) {
       if(!flag) {
