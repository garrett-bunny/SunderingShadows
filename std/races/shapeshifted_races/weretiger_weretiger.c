...
int default_descriptions(object obj)
{
    ...
    obj->set_description("is a strange humanoid resembling its original race. It has tiger-like ears and is covered in stripped fur, a tail follows behind.");
...
int change_into_message(object obj)
{
    ...
    tell_object(obj, "%^RESET%^%^RED%^You can feel your body beginning to change, you grow a tail and couple of tiger-like ears!");
    tell_object(obj, "%^RESET%^%^RED%^%^BOLD%^Your senses heighten, you can feel the pulse of the night, smell countless colors, you can taste the very air. You are weretiger!");

    tell_room(environment(obj), "%^RESET%^%^RED%^" + obj->QCN + " grows very still and appears to concentrate deeply.", obj);
    tell_room(environment(obj), "%^RESET%^%^RED%^" + obj->QCN + " begins to change in front of your very eyes and grows a tail and couple of tiger-like ears!", obj);
    tell_room(environment(obj), "%^RED%^Where " + obj->QCN + " once stood, is now a weretiger!", obj);
...
int init_shape(object obj, string str)
{
    ...
    shape->set_shape_race("weretiger");
    shape->apply_bonuses(shape->query_owner());

    obj->add_id("weretiger");
