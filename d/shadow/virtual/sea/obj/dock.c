...
int retrieve(string str)
{
   ...
   object ob;
    ...
    //should not have to keep removing/writing files now, the boat
    //file simply inherits a boat_inherit, so every individual
    //boat simply inherits code - Saide, December 2016
    if(file_size(file) != file_size(BOAT))
    {
        rm(file);
