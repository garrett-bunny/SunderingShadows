...
void impaler(object targ){
   ...
   if(dam=="impales"){
      targ->cause_typed_damage(targ, "torso", roll_dice(10,20), "piercing");
...
void tornado(object targ){
   ...
   if(SAVING_THROW_D->reflex_save(targ, 55)){
      ...
      tell_room(ETO,"Callista kneels and touches her palms against the floor.");
	  tell_room(environment(targ),"%^CYAN%^The ground beneath your feet shakes violently");
...
void light(object targ){
   ...
   if(SAVING_THROW_D->will_save(targ, 55)){
	  ...
	  tell_room(ETO,"%^BOLD%^%^BLACK%^Callista lowers her hands and blasts a horrible beam of black light at "+targ->QCN+"!",targ);
	  tell_object(targ,"%^BOLD%^%^BLACK%^Callista lowers her hands and blasts you with a horrible beam of black light!");
	  tell_object(targ,"%^RED%^It utterly destroys you!");
      targ->cause_typed_damage(targ, "torso", random(400)+100, "void");
	  return;
   }
   else{
      tell_object(targ,"%^YELLOW%^Callista grabs you by the throat and slams you against the wall!");
      targ->cause_typed_damage(targ, "torso", random(170)+50, "bludgeoning");
	  tell_room(ETO,"%^YELLOW%^Callista grabs "+targ->QCN+" and slams "+targ->QP+" head against the wall.",targ);
   }
}
