  ...
  set_menu(
   ...
   set_your_mess(
    ...
    set_menu_long(
        ...
        "%^RED%^A thick backdrop of tomato stew complements the light flavors of %^RESET%^fish %^RED%^and %^RESET%^shellfish%^RED%^ in this delightful concoction.  The aroma of %^YELLOW%^saffron%^RESET%^%^RED%^ rises from the steaming bowl, bringing with it a hint of %^BOLD%^wine%^RESET%^%^RED%^.%^RESET%^\n",
        ...
        "Made from %^BOLD%^white wine%^RESET%^, this fruity drink has been filled with sliced %^MAGENTA%^red %^RESET%^and %^GREEN%^green %^RESET%^grapes as well as sliced %^ORANGE%^peaches%^RESET%^.  Sweetened with %^YELLOW%^lemonade %^RESET%^and %^BOLD%^sugar%^RESET%^, it is further strengthened with a splach of %^ORANGE%^peach-flavored brandy%^RESET%^.\n",
