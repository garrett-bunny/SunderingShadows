...
void create() 
{
    ...
    "this doorway will take you %^BOLD%^%^RED%^upstairs%^BOLD%^%^CYAN%^"+
...
int enter_function(string what) 
{
    ...
    if(!stringp(dest)) 
    {
        tell_object(TP, "ERROR: Something went wrong with SECOND DOOR.");
        return 1;
    }
    TP->move(dest);
    if(!TP->query_invis())
...
varargs void pick_room(string file)
{
    ...
    if(!stringp(file)) rooms = get_dir(HHROOMS+"ff*") - get_dir(HHROOMS+"ffr*") - get_dir(HHROOMS+"ffiw*");
    if(objectp(ETO)) 
    {
    ...
    {
        //tell_object(environment(previous_object()), "%^B_CYAN%^%^YELLOW%^ERROR: room for door 2 FAILED to LOAD%^RESET%^");
        return;
    }
    if(!objectp(MyRoom)) 
    {
    ...
    SAVE_D->remove_array("hhouse_entry_ob2");
    SAVE_D->set_value("hhouse_entry_ob2", TO);
    
    if(room != file) 
    {
        SAVE_D->remove_array("hhouse_entry_ob2_location");
        SAVE_D->set_value("hhouse_entry_ob2_location", room);
    }
    
    rooms = get_dir(HHROOMS+"sfh*");
