...
void create(){
   ::create();
   set_id(({"sword","bastard","bastard sword","weapon","Sword","Weapon","Bastard"}));
...
int wieldit() {
   ...
   if(ETO->query_property("good item")) {
     tell_object(ETO,"%^BOLD%^%^BLACK%^The spirit within the sword snarls at the good items you're wearing!%^RESET%^");
     return 0;
   }
...
int unwieldit() {
   tell_object(ETO,"%^BOLD%^%^CYAN%^The energy of the spirit fades as you unwield the sword.%^RESET%^");
   tell_room(environment(ETO),""+ETO->QCN+"s%^BOLD%^%^CYAN%^ sword stop glowing as "+ETO->QS+" unwields it.%^RESET%^",ETO);
   ETO->set_property("evil",-1);
   ETO->set_property("soth weapon",-1);
   return 1;
}
...
// int counter, i, j;
      ...
      case 0..5:
         ...
         case 13..15:
            ...
            tell_object(ETO,"%^BOLD%^%^RED%^The sword releases the spirit's wrath upon "+ob->QCN+"%^RESET%^");
            tell_object(ob,"%^BOLD%^%^RED%^"+ETO->QCN+"'s sword releases its spirit upon you!%^RESET%^");
            tell_room(environment(query_wielded()),"%^BOLD%^%^RED%^"+ETO->QCN+"'s sword releases its spirits wraith upon "+ob->QCN+"!%^RESET%^",({ETO,ob}));
            counter = 0;
			set_property("magic",1);
