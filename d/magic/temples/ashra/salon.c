...
void create()
{
    ...
    set_items(([			
        ...
        ({"tables", "table" }) : "%^ORANGE%^Long low finely lacquered tables are set near each couch, each with a vase containing several %^BOLD%^%^BLACK%^black roses.%^RESET%^",
        ({"staircases", "staircase", "stairs"}) : "%^BOLD%^%^BLACK%^These curving staircases wind up to the balcony of the second floor, the soft %^RESET%^%^MAGENTA%^violet carpet %^BOLD%^%^BLACK%^flowing down them like a waterfall. The adamantine rails are gilded in %^WHITE%^silver %^BLACK%^and a pair of large %^RESET%^%^MAGENTA%^amethysts %^BOLD%^%^BLACK%^are embedded deeply at their base.%^RESET%^",
    ...
    set_exits(([
        ...
        "north" : "/d/magic/temples/ashra/ashra",
        "up" : "/d/magic/temples/ashra/balcony",
    ]));
    set_door("dark doors","/d/magic/temples/ashra/hall","south",0);
