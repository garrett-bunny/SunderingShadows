...
%^CYAN%^You are standing in a small lagoon of crystal clear %^BOLD%^water %^RESET%^%^CYAN%^between Aramanth Island and a sandbar to the west. It mpties out into wast o%^BOLD%^%^WHITE%^ce%^RESET%^%^CYAN%^a%^BOLD%^n%^RESET%^%^CYAN%^ here. To the east you can see jagged terrain fo the island. A large rock %^RED%^f%^WHITE%^o%^RED%^rmat%^WHITE%^i%^RED%^on %^CYAN%^rises from the water just east of you and casts shadow over you.  The water is cool and refreshing as it flows past you.%^WHITE%^
");
   set_listen("default", "The ocean waves are somewhat relaxing as they lap against the sand..");
       set_exits(([
    "north" : "/d/islands/common/aramanth/room31",
    "east" : "/d/islands/common/aramanth/room36",
]));
       set_items(([
