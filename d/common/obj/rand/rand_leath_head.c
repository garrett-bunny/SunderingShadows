...
#define types ({"coif","skullcap","hat","mask","bandanna","hood"})
...
void create(){
	...
	set_property("enchantment",2);
      //set_type("leather"); changing this so that each type has its own type.  Cloth is cloth, leather is leather
   	set_limbs(({"head"}));
   	set_size(random(3)+1); 
}
