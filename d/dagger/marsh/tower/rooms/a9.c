...
int exita_fun() {
    ...
    write("%^GREEN%^As you head to the southwest, you hear a faint *click* beneath "
	"your feet.%^RESET%^"
    );
    return 1;
}
int exitb_fun() {
    ...
    if(FLAG == 0) {
        write(
	    "%^BOLD%^%^BLUE%^A shadow leaps from the wall and runs to the west!%^RESET%^");
        say(
	    "%^BOLD%^%^BLUE%^"+TPQCN+
	    " looks startled and heads quickly west.%^RESET%^"
	,TP);
        return 1;
    } else {
	return 1;
