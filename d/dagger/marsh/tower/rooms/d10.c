...
void create() {
    ...
    set_listen("default",
        "The silence here is unbearable, so loud in negative energy it throbs deep inside your temples."
    );
    set_smell("default",
        "You smell the herbs and petals; sweet and yet sickly."
    );
    set_items(([
...
int exita_fun() {
    ...
    write("%^CYAN%^As you head to the northwest, a "
	"%^BOLD%^series of chilled icedarts%^RESET%^%^CYAN%^ "
	"strike you!%^RESET%^"
    );
    say("%^CYAN%^As "+TPQCN+" heads northwest, a "
	"%^BOLD%^series of chilled icedarts%^RESET%^%^CYAN%^ "
	"strike "
	+TP->query_possessive()+" "
	"body!%^RESET%^"
