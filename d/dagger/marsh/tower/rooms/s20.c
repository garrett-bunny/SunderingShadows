...
set_short("%^BLUE%^The Shadowland");
	...
	"%^RESET%^%^CYAN%^.%^RESET%^\n"
	"%^BOLD%^%^RED%^An enormous %^BLUE%^black archway%^RED%^ is set into the fog to the north.%^RESET%^"
    );
    set_listen("default",
	...
	"archway":"%^BOLD%^%^BLUE%^Going through the archway is the only visible means of travelling further to the "
	"north. The black archway is covered in %^MAGENTA%^ancient glyphs%^BLUE%^. A %^YELLOW%^shimmering haze%^BLUE%^ "
	"is slightly visible across the entire face of the archway.",
    ]));
    set_exits(([
	"south": RPATH "s12",
	"north": RPATH "s16",
...
int exitb_fun() {
    write(
        "%^BLUE%^You are prevented from moving north by an invisible force.%^RESET%^"	
    );
    say(
 	"%^BLUE%^"+TPQCN+" tries to pass through the arch to the north, but fails.%^RESET%^"
    ,TP);
    return 0;
}
