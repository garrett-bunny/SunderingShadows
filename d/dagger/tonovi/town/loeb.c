...
void heart_beat()
{
    ...
    else
    {
        if(!(time() % 5))
        {
            victim = shuffle(query_attackers())[0];
            
            if(!victim->reflex_save(LVL))
            {
                tell_room(environment(this_object()), "%^YELLOW%^Loeb lunges forward and SLAMS " + victim->QCN + " with his shield!%^RESET%^");
                victim->cause_typed_damage(victim, victim->return_target_limb(), roll_dice(LVL, 6), "bludgeoning");
                victim->set_tripped(roll_dice(1,6),"%^RESET%^%^YELLOW%^You are struggling to get your feet back under you!");
            }
            else
                tell_room(environment(this_object()), "Loeb tries to slam " + victim->QCN + " with his shield but misses!");
        }
    }
}
